...
allowed-tools: Bash(git:*), Read, Edit, mcp__sequential-thinking__sequentialthinking
---
...
**A**ction: Complete Git Flow branch with type detection (feature/release/hotfix), pre-merge validation (clean working directory, tests passing, no conflicts), merge to appropriate targets (feature → develop, release/hotfix → main + develop), create tags (release/hotfix only), delete branches (unless --no-delete), push all changes
...
**Target**: 85+ overall (Completeness >95% finish workflow, Accuracy >90% merge targets, Relevance >85% quality gates, Efficiency <30s typical finish)
