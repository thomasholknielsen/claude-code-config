...
description: "MUST BE USED for database architecture analysis - provides microservices data patterns, polyglot persistence strategy, database technology selection, sharding strategies, CQRS/event sourcing, and scalability planning. This agent conducts comprehensive database architecture analysis and returns actionable recommendations. It does NOT implement changes - it only analyzes database architecture and persists findings to .agent/Session-{name}/context/database-architecture-analyst.md files. Invoke when: keywords 'database architecture', 'microservices data', 'polyglot persistence', 'database selection', 'sharding', 'CQRS', 'event sourcing', 'database scalability'."
