...
- **MUST persist to `.agent/context/{session-id}/mobile-react-native-analyst.md`**
...
Persist comprehensive React Native analysis to `.agent/context/{session-id}/mobile-react-native-analyst.md` using XML-tagged structure. Return concise 2-3 sentence summary with React Native version, Expo vs bare workflow, top performance issue (FlatList, images, bundle size), and artifact reference.
