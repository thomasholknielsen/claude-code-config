...
- **MUST persist findings to `.agent/context/{session-id}/database-sql-analyst.md`** - Required for main thread access
...
Persist comprehensive SQL analysis to `.agent/context/{session-id}/database-sql-analyst.md` using XML-tagged structure. Return concise 2-3 sentence summary with query performance score, critical slow queries, optimization opportunities (with speedup estimates), and artifact reference.
